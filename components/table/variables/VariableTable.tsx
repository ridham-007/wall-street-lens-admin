...
const VariableTable = (props: TableProps) => {
    ...
    const [deleteVariableMapping] = useMutation(DELETE_VARIABLE_MAPPINGS_BY_COMPANY);

    const onDeleteVariable = async (id: any, company: any) => {
        setShowLoader(true);
        await deleteVariableMapping({
            variables: {
                mappingId: id,
            }
        })
        props.refetchMasterVariables();
        setShowLoader(false);
    }

    const onUpdateRelation = async (perameters: any, id: string) => {
        setShowLoader(true);
    ...
    return <>
                                    ...
                                    <>
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                onClick={() => { setDeletePopup(true); setDeleteId(current?.id?.toString() || ''); setCurrentData(current) }}
                                            >
        ...
        {show && (<AddVariable  data={currentData} onClose={() => { setShow(false); setCurrentData({}); setDeleteId('') }} onSuccess={onUpdateRelation} />)}
        {deletePopup && <DeleteVariable id={deleteId} onSuccess={onDeleteVariable} onClose={() => {
            setDeleteId('');
...
function AddVariable(props: AddUpdateParameterProps) {
    ...
    const [subIndustry, setSubIndustry] = useState(props?.data?.subIndustry || "");
    ...
    const handleOnSave = () => {
        ...
        props.onSuccess && props.onSuccess({ industry, subIndustry, company, id: props?.data?.masterVariable?.id }, props?.data?.id);
...
function DeleteVariable(props: DeleteVariableProps) {
    const handleOnSave = () => {
        ...
        props.onSuccess && props.onSuccess(props?.id, props?.data?.company)
        props.onClose && props.onClose()
    };
