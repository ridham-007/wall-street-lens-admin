...
export default function Variable({
	...
	const onAdd = async () => {
		const tableView: TableView = termView?.getViewForTerm ?? undefined;
		if (!tableView) {
			return;
		}

		const quarterId = tableView.quarterId;
		setShowLoader(true);
		await addRow({
			variables: {
				quarterId: quarterId,
	...
	let selectedColumn;
	if (!!cellData && cellData.columnIndex) {
		selectedColumn = headers[cellData.columnIndex];
	}
